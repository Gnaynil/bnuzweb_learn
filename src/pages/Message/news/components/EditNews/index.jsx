...
import { message, Form, Select, Input, Upload, Button, Card, Modal } from "antd";
...
const News = props => {
  ...
  const newsValue = JSON.parse(localStorage.getItem("ItemNews"));
  ...
  //富文本上传媒体
  const [editorState, SetEditorState] = useState(BraftEditor.createEditorState(newsValue.content));
  ...
  return (
    <PageContainer
      ...
      extra={
        <div>
          <Button type="primary" onClick={BackToNews}>返回上一页</Button>
        </div>
      }>
      <Card style={{ minWidth: 1500 }}>
        <Form
          {...formLayout}
          name="basic"
          form={form}
          ...
          initialValues={{
            ...newsValue,
            content: BraftEditor.createEditorState(newsValue.content),
          }}
        >
          <Form.Item
            label="ID"
